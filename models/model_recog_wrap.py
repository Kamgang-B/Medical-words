...
class ModelRecog():
    ...
    def z_define_graph_all(graph, train = True): # learn.ModeKeys.TRAIN  INFER
        ...
        with graph.as_default():
            ...
            #
            print(' ')
            print(result_logits.op.name)    # recog_logits/Sigmoid   /BiasAdd
    ...
    def validate(self, data_valid, step):
        ...
        with self.graph.as_default():
            ...
            with tf.Session(config = self.z_sess_config) as sess:
                ...
                result_logits = self.graph.get_tensor_by_name('rnn_logits/BiasAdd:0')
                #
                ...
                num_right = 0
                num_batches = np.ceil(num_samples * 1.0 / self.z_batch_size_valid)
                #
                curr = 0
                batch_start = 0
                batch_end = self.z_batch_size_valid
                while batch_start < num_samples:
                    ...
                    #
                    batch_start = batch_start + self.z_batch_size_valid
                    batch_end = min(batch_end + self.z_batch_size_valid, num_samples)
                    #
                    ...
                    print(trans)
                    #
                    diff = [x[0]-x[1] for x in zip(targets[0], trans[0])]
                    num_right += diff.count(0)
                    #                
                #
                acc = num_right/ (num_samples * meta.num_chars)
                #
